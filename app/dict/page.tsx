...
const NAVER_DICT_URL = "https://en.dict.naver.com/";

export default function DictPage() {
  const [showFallback, setShowFallback] = useState(false);

  useEffect(() => {
    // iframe 차단 시 onError가 안 뜨는 경우가 많아, 잠시 후 안내를 함께 노출
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex shrink-0 items-start justify-between gap-2 border-b border-border/60 px-3 py-2">
        ...
        <a
          ...
          className={cn(
            "inline-flex h-8 shrink-0 items-center gap-1 rounded-lg border border-border bg-background px-2 text-xs font-semibold touch-manipulation hover:bg-muted"
          )}
        >
